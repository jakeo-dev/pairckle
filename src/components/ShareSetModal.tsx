  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.error("turnstile error:", data.message);
        turnstileRef.current?.reset();
      }
    } catch (error) {
    ...
    >
      ...
      >
        ...
        <div className="float-right mt-6 flex gap-2">
          <button
            className={`rounded-full border-2 border-neutral-400/40 bg-transparent px-5 py-2 text-sm transition hover:bg-neutral-400/20 active:bg-neutral-400/30 md:text-base`}
            onClick={(e) => {
              e.preventDefault();
